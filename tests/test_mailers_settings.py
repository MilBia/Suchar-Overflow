...
if any deprecated ``EMAIL_*`` name is also set, so re-introducing one would
already break the whole suite. These tests additionally pin that the migrated
config is present and functional, and that sending mail is warning-clean.
"""
...
def test_no_deprecated_email_setting_is_overridden() -> None:
    overridden = sorted(
