"""The "Publika Rozgrzana" hidden achievement — data migration 0023 (#296).

Pure data check: the migration's ``create_publika_rozgrzana_achievement`` must
seed one hidden ``FRONTEND`` / ``FRONTEND_EVENT`` achievement with both the pl
and en translation columns populated (django-modeltranslation).
...
_migration = importlib.import_module(
    "suchar_overflow.achievements.migrations.0023_publika_rozgrzana_achievement_data",
)


@pytest.fixture
def _seed_publika_rozgrzana() -> None:
    Achievement.objects.filter(slug=PUBLIKA_ROZGRZANA_SLUG).delete()
    _migration.create_publika_rozgrzana_achievement(global_apps, None)


@pytest.mark.django_db
@pytest.mark.usefixtures("_seed_publika_rozgrzana")
...
def test_migration_populates_pl_and_en_translations() -> None:
    ...
    # attribute access.
    row = (
        Achievement.objects.filter(slug=PUBLIKA_ROZGRZANA_SLUG)
        .values(
            "name_pl",
            "name_en",
            "description_pl",
    ...
    assert row["name_pl"] == "Publika Rozgrzana"
    assert row["name_en"] == "Warmed-Up Crowd"
...
def test_migration_is_idempotent() -> None:
    ...
    assert Achievement.objects.filter(slug=PUBLIKA_ROZGRZANA_SLUG).count() == 1
