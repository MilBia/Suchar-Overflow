"""The "Niezdecydowany" hidden achievement — data migration 0021 (#289).

Pure data check: the migration's ``create_niezdecydowany_achievement`` must
...
_migration = importlib.import_module(
    "suchar_overflow.achievements.migrations.0021_niezdecydowany_achievement_data",
)


@pytest.fixture
def _seed_niezdecydowany() -> None:
    Achievement.objects.filter(slug=NIEZDECYDOWANY_SLUG).delete()
    _migration.create_niezdecydowany_achievement(global_apps, None)


@pytest.mark.django_db
@pytest.mark.usefixtures("_seed_niezdecydowany")
...
def test_migration_populates_pl_and_en_translations() -> None:
    ...
    # attribute access.
    row = (
        Achievement.objects.filter(slug=NIEZDECYDOWANY_SLUG)
        .values(
            "name_pl",
            "name_en",
            "description_pl",
    ...
    assert row["name_pl"] == "Niezdecydowany"
    assert row["name_en"] == "Undecided"
...
def test_migration_is_idempotent() -> None:
    ...
    assert Achievement.objects.filter(slug=NIEZDECYDOWANY_SLUG).count() == 1
